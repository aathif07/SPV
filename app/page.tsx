...
import { VisionIcon, type VisionIconName } from "./components/vision-icon";
...
const vision: [string, string, VisionIconName][] = [
  ["Strong Infrastructure", "Better roads, transport, water systems and connectivity.", "infrastructure"],
  ["Opportunities for Youth", "Education, skills, employment and entrepreneurship.", "youth"],
  ["Education for the Future", "Quality learning for a rapidly changing world.", "education"],
  ["Healthcare for Every Family", "Accessible, dependable care for every community.", "healthcare"],
  ["Empowering Women", "Safety, opportunity, leadership and financial independence.", "women"],
  ["Strong Rural Communities", "Infrastructure, amenities and local employment.", "rural"],
  ["Agriculture & Farmers", "Stronger support and opportunity for farming communities.", "agriculture"],
  ["Technology & Innovation", "Startups, digital infrastructure and new industries.", "technology"],
];
...
export default function Home() {
  return <main>
    ...
    <section className="section vision-section" id="vision"><div className="vision-intro"><SectionHead label="Vision for Tamil Nadu" title="Building a Stronger Tamil Nadu" /><div className="prose"><p>Economically strong, socially inclusive and full of opportunity for every generation.</p></div></div><div className="vision-grid" role="region" aria-label="Vision priorities. Scroll horizontally to explore all cards.">{vision.map(([title,text,icon],i)=><article className={i===0||i===7?"featured":""} key={title}><div className="vision-card-top"><span className="card-no">V / {String(i+1).padStart(2,"0")}</span><VisionIcon name={icon} /></div><h3>{title}</h3><p>{text}</p></article>)}</div></section>

    <section className="constituency"><div className="constituency-image"><span aria-hidden="true">THONDAMUTHUR</span><Image className="constituency-emblem" src="/images/aiadmk-emblem-transparent.png" alt="" aria-hidden="true" width={1254} height={1254} /><Image className="constituency-photo" src="/images/sp-velumani-studio-namaste-cutout.png" alt="S. P. Velumani greeting with folded hands" fill sizes="(max-width: 800px) 100vw, 50vw" /></div><div className="constituency-copy"><SectionHead label="Thondamuthur" title="A Constituency Close to the Heart" /><p>Thondamuthur remains central to <NameMark className="inline" />'s political journey and grassroots engagement.</p><ul>{["Development activities","Public interactions","Constituency visits","Community programmes"].map(x=><li key={x}>{x}</li>)}</ul><a className="button button-primary" href="#gallery">Explore Thondamuthur <span>↗</span></a></div></section>

    <section className="section news-section"><SectionHead label="News & Updates" title={<>Latest From <NameMark className="heading" /></>} /><div className="news-intro"><p className="news-lead">Stay connected with the latest activities, political programmes, public meetings, development initiatives and statements.</p><h3 className="sub-label">Categories · 05</h3><a className="button button-primary news-all-cta" href="/blog">Read All Posts <span>↗</span></a></div><div className="news-grid">{news.map(([title,text],i)=><article className={i===0?"news-featured":""} key={title}><Image className="news-leaf-watermark" src="/images/aiadmk-emblem-transparent.png" alt="" aria-hidden="true" width={1254} height={1254} /><div className="news-card-top"><span>{String(i+1).padStart(2,"0")}</span><span className="news-leaf-mark"><Image src="/images/aiadmk-emblem-transparent.png" alt="" aria-hidden="true" width={1254} height={1254} /></span></div><div className="news-card-copy"><h3>{title}</h3><p>{text}</p></div><b aria-hidden="true">↗</b></article>)}</div></section>

    <section className="gallery-section" id="gallery"><div className="gallery-head"><SectionHead label="Media & Gallery" title="Moments of Leadership" /><p>Public moments from across his political journey, community engagement and party programmes.</p></div><div className="gallery-grid"><figure className="gallery-tall gallery-cutout"><Image className="gallery-cutout-photo" src="/images/sp-velumani-namaste-cutout.png" alt="S. P. Velumani greeting with folded hands" fill sizes="(max-width: 800px) 100vw, 48vw" /></figure><figure><Image src="/images/spv-studio-namaste.jpg" alt="S. P. Velumani greeting with folded hands" fill sizes="(max-width: 800px) 50vw, 25vw" /></figure><figure><Image src="/images/spv-studio-victory-scarf.jpg" alt="S. P. Velumani raising a victory sign in the party scarf" fill sizes="(max-width: 800px) 50vw, 25vw" /></figure><figure><Image src="/images/spv-studio-seated.jpg" alt="S. P. Velumani seated portrait" fill sizes="(max-width: 800px) 50vw, 25vw" /></figure><figure><Image src="/images/spv-studio-namaste-scarf-full.jpg" alt="S. P. Velumani greeting with folded hands in the party scarf" fill sizes="(max-width: 800px) 50vw, 25vw" /></figure><figure><Image src="/images/spv-studio-victory.jpg" alt="S. P. Velumani raising a victory sign" fill sizes="(max-width: 800px) 50vw, 25vw" /></figure><figure><Image src="/images/spv-studio-seated-smile.jpg" alt="S. P. Velumani smiling in a seated portrait" fill sizes="(max-width: 800px) 50vw, 25vw" /></figure></div><div className="gallery-categories"><h3>Gallery Categories</h3>{["Political Events","Public Meetings","People's Interactions","Government Activities","Constituency Visits","Party Programmes","Social & Community Events","Speeches","Media Interviews"].map(x=><span key={x}>{x}</span>)}</div><p className="media-note">Official photographs of S. P. Velumani.</p></section>
